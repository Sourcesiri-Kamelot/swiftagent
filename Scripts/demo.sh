#!/bin/bash
# SwiftAgent Toolkit - Demo Script
...
echo -e "${BLUE}🤖 SwiftAgent Toolkit Demo${NC}"
echo -e "${BLUE}Let's see what your free AI assistant can do!${NC}"
echo ""

# Check if swiftagent is available
if ! command -v swiftagent &> /dev/null; then
    echo -e "${RED}❌ SwiftAgent Toolkit not found!${NC}"
...
setup_demo_files() {
    ...
    mkdir -p ~/Documents/swiftagent-demo
    
    # Create a sample text file
    cat > ~/swiftagent-demo/sample.txt << 'EOF'
SwiftAgent Toolkit Demo File
...
EOF
    ...
    if command -v convert &> /dev/null; then
        convert -size 400x200 xc:lightblue -pointsize 20 -gravity center \
                -draw "text 0,0 'SwiftAgent Toolkit
Demo Image'" \
                ~/Documents/swiftagent-demo/demo.png
    else
        # Create a simple text file as image placeholder
        echo "This would be an image file in a real demo" > ~/Documents/swiftagent-demo/demo.png
    fi
    
    echo -e "${GREEN}✅ Demo files created in ~/Documents/swiftagent-demo/${NC}"
    echo ""
}
...
demo_chat() {
    run_demo \
        "AI Chat" \
        "swiftagent chat 'Write a short poem about artificial intelligence'" \
...
demo_files() {
    run_demo \
        "File Reading" \
        "swiftagent read ~/Documents/swiftagent-demo/sample.txt" \
...
demo_write() {
    run_demo \
        "File Writing" \
        "swiftagent write ~/Documents/swiftagent-demo/ai_response.txt 'This is a response generated by the AI assistant. The SwiftAgent Toolkit makes it easy to work with files and get AI-powered insights.'" \
...
demo_list() {
    run_demo \
        "Directory Listing" \
        "swiftagent list ~/Documents/swiftagent-demo" \
...
demo_search() {
    run_demo \
        "File Search" \
        "swiftagent search 'demo' ~/Documents" \
...
demo_interactive() {
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}🎯 Demo: Interactive Mode${NC}"
...
show_help() {
    ...
    echo "  swiftagent chat 'your message'     # Quick chat with AI"
    echo "  swiftagent                         # Start interactive mode"
    echo ""
    echo -e "${BLUE}File Commands:${NC}"
    echo "  swiftagent read file.txt           # Read and analyze a file"
    echo "  swiftagent write file.txt 'text'   # Write content to a file"
    echo "  swiftagent list directory          # List files in directory"
    echo "  swiftagent search 'text' dir       # Search for files"
    echo ""
    echo -e "${BLUE}Image Commands:${NC}"
    echo "  swiftagent photo image.jpg         # Analyze an image"
    echo "  swiftagent text image.png          # Extract text from image"
    echo "  swiftagent describe image.jpg      # Get AI description"
    echo ""
    echo -e "${BLUE}System Commands:${NC}"
    echo "  swiftagent status                  # Check system status"
    echo "  swiftagent providers               # List available AI providers"
    echo "  swiftagent help                    # Show this help"
    echo ""
}

# Main demo flow
main() {
    echo -e "${GREEN}Welcome to the SwiftAgent Toolkit Demo!${NC}"
    ...
    echo "• Explore the help system: swiftagent help"
    echo "• Join our community for support"
    echo ""
    echo -e "${BLUE}Happy AI-ing! 🚀${NC}"
}
